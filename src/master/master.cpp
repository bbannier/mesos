    ...
    if (!slave->resourceProviders.contains(providerId)) {
      ...
      foreach (
        ...
        if (!protobuf::isTerminalState(operation.latest_status().state())) {
          ...
          // at this point. Similarly, we cannot track operations not
          // tied to a particular framework.
          //
          // TODO(bbannier): Consider introducing ways of making sure
          // an agent always knows the `FrameworkInfo` of operations
          // triggered on its resources, e.g., by adding an explicit
          // `FrameworkInfo` to operations like is already done for
          // `RunTaskMessage`, see MESOS-8582.
          if (framework == nullptr) {
            if (operation.has_framework_id()) {
              LOG(WARNING)
