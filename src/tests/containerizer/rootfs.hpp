...
public:
  ...
  // Add a host directory or file to the root filesystem. If
  // the given host path is a symlink, both the link target
  // and the link itself will be copied into the root.
...
private:
  Try<Nothing> copyPath(const std::string& path);
};
