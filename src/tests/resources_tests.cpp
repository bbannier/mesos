...
TEST(DiskResourceTest, IdAddition)
{
  ...
  EXPECT_EQ(1u, (r1 + r2).count(disk1));
  EXPECT_EQ(1u, (r1 + r2).count(disk2));

  EXPECT_EQ(0u, (r2 + r2).count(disk2));
  EXPECT_EQ(2, (r2 + r2).get<Value::Scalar>("disk")->value());

  EXPECT_EQ(1u, (r2 + r3).count(disk2));
  EXPECT_EQ(1u, (r2 + r3).count(disk3));

  EXPECT_EQ(0u, (r3 + r3).count(disk3));
  EXPECT_EQ(2, (r3 + r3).get<Value::Scalar>("disk")->value());

  EXPECT_EQ(1u, (r3 + r4).count(disk3));
  EXPECT_EQ(1u, (r3 + r4).count(disk4));
}
...
TEST(ResourcesOperationTest, CreateVolume)
{
  ...
  create_volume.set_type(Offer::Operation::CREATE_VOLUME);
  create_volume.mutable_create_volume()->mutable_source()->CopyFrom(source);
  create_volume.mutable_create_volume()->set_target_type(
      Resource::DiskInfo::Source::PATH);
  ...
  destroy_volume.set_type(Offer::Operation::DESTROY_VOLUME);
  destroy_volume.mutable_destroy_volume()->mutable_volume()->CopyFrom(volume);

  EXPECT_SOME_EQ(
      (other + source),
      (other + volume).apply(destroy_volume));
...
TEST(ResourcesOperationTest, CreateBlock)
{
  ...
  create_block.set_type(Offer::Operation::CREATE_BLOCK);
  create_block.mutable_create_block()->mutable_source()->CopyFrom(source);
  ...
  destroy_block.set_type(Offer::Operation::DESTROY_BLOCK);
  destroy_block.mutable_destroy_block()->mutable_block()->CopyFrom(block);

  EXPECT_SOME_EQ(
      (other + source),
      (other + block).apply(destroy_block));
