...
TEST_F(HierarchicalAllocatorTest, UpdateSlaveTotalResources)
{
  ...
  allocator->updateSlave(agent.id(), None(), agentResources2);
  ...
  // the agent.
  allocator->updateSlave(agent.id(), None(), agent.resources());
...
TEST_F(HierarchicalAllocatorTest, UpdateSlaveCapabilities)
{
  ...
  allocator->updateSlave(agent.id(), None(), None(), AGENT_CAPABILITIES());
