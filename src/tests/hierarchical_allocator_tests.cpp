...
TEST_F(HierarchicalAllocatorTest, OfferFilter)
{
  ...
  string filterCountEndpoint = strings::join(
      "/",
  ...
  metrics = Metrics();
  EXPECT_EQ(0, metrics.values[filterCountEndpoint]);
}
...
TEST_F(HierarchicalAllocatorTest, RemoveQuota)
{
  ...
  JSON::Object metrics = Metrics();
  string metric =
    "allocator/mesos/quota/roles/" + QUOTA_ROLE + "/cpus/offered_or_allocated";
  EXPECT_EQ(0u, metrics.values.count(metric));
  metric =
    "allocator/mesos/quota/roles/" + QUOTA_ROLE + "/mem/offered_or_allocated";
  EXPECT_EQ(0u, metrics.values.count(metric));
}
...
TEST_F(HierarchicalAllocatorTest, DRFWithQuota)
{
  ...
  JSON::Object metrics = Metrics();
  string metric =
    "allocator/mesos/quota/roles/" + QUOTA_ROLE + "/cpus/guarantee";
  EXPECT_EQ(0.25, metrics.values[metric]);
  metric = "allocator/mesos/quota/roles/" + QUOTA_ROLE + "/mem/guarantee";
  ...
  EXPECT_EQ(128, metrics.values[metric]);
  metric =
    "allocator/mesos/quota/roles/" + QUOTA_ROLE + "/disk/offered_or_allocated";
  EXPECT_EQ(0u, metrics.values.count(metric));
...
TEST_F(HierarchicalAllocatorTest, ResourceMetrics)
{
  ...
  JSON::Object metrics = Metrics();
  EXPECT_EQ(2, metrics.values["allocator/mesos/cpus/total"]);
  EXPECT_EQ(1024, metrics.values["allocator/mesos/mem/total"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/disk/total"]);

  // No frameworks are registered yet, so nothing can be allocated.
  EXPECT_EQ(0, metrics.values["allocator/mesos/cpus/offered_or_allocated"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/mem/offered_or_allocated"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/disk/offered_or_allocated"]);
  ...
  // `framework`.
  metrics = Metrics();
  EXPECT_EQ(2, metrics.values["allocator/mesos/cpus/offered_or_allocated"]);
  EXPECT_EQ(1024, metrics.values["allocator/mesos/mem/offered_or_allocated"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/disk/offered_or_allocated"]);
  ...
  metrics = Metrics();
  EXPECT_EQ(0, metrics.values["allocator/mesos/cpus/total"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/mem/total"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/disk/total"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/cpus/offered_or_allocated"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/mem/offered_or_allocated"]);
  EXPECT_EQ(0, metrics.values["allocator/mesos/disk/offered_or_allocated"]);
...
TEST_F(HierarchicalAllocatorTest, AllocationRunsMetrics)
{
  ...
  JSON::Object metrics = Metrics();
  EXPECT_EQ(0, metrics.values["allocator/mesos/allocation_runs"]);
  ...
  metrics = Metrics();
  string metricKey =
    strings::join("/", "allocator/mesos/framework_allocations", framework.id());
  EXPECT_EQ(1, metrics.values[metricKey]);

  EXPECT_EQ(allocations, metrics.values["allocator/mesos/allocation_runs"]);
}
